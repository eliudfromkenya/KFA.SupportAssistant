...
internal static class Functions
{
  ...
  public static Dictionary<string, DataTable> InitialData { get; internal set; }
  ...
  static string singularize(string name)
  {
    ...
    name = name?.Singularize()?.Replace("Datum", "Data");
    //if (name.Contains(" "))
  ...
  internal static string GetRelName(TableColumn[] tableColumns, DatabaseTable master, string colName)
  {
    ...
    Func<string, bool> CheckIfValid = name =>
    {
      ...
      if (new[] { "narration", "descri" }.Any(c => true == name?.Contains(c)))
        return false;
      return true;
    };
